...
class User(Model):
	...
	def insert_likes(self,like):
		...
		like_id = self.db.query_db("SELECT like_id FROM facebook_likes WHERE name='{}' ORDER BY like_id LIMIT 1".format(like))
		user_like_id = self.db.query_db("SELECT user_like_ids FROM user_has_likes WHERE user_like_ids='{}' AND like_user_ids='{}'".format(like_id[0]['like_id'],1))
		if user_like_id==[]:
			query2 = "INSERT INTO user_has_likes (like_user_ids,user_like_ids) VALUES ('{}','{}')".format(1,like_id[0]['like_id'])
